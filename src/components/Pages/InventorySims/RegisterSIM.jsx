...
const ADD_SIM = gql`
  mutation addSimDetailsFromPartnerAdmin(
    $phoneNumber: String!
    $simNumber: String!
...
class AddSim extends Component {
  ...
  //   e.preventDefault()
  //   addProvider({
  //     variables: { type: this.state.simprovider },
  //     refetchQueries: [`allSimDetails`]
  //   })
  ...
  handleSubmit = addSim => e => {
    ...
    addSim({
      ...
      // refetchQueries: [`allSimDetails`]
      // refetchQueries: [
      //   { query: this.props.ALL_SIM_DETAILS },
      //   { query: this.props.allSimDetails }
      // ]
      refetchQueries: ['allSimDetails', 'getPartnerSimStockByServiceProvider']
    })
      .then(() => {})
      .catch(err => {
        this.setState({ err })
              ...
              {(addSim, { data, error: simError }) => (
                <div>
                  ...
                  >
                    ...
                    {simError &&
                      simError.graphQLErrors.map(({ message }, i) => (
